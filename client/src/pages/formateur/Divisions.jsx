    ...
    useEffect(() => {
        const fetchUsers = async () => {
            ...
            try {
                const token = localStorage.getItem('token');
                const config = { headers: { Authorization: `Bearer ${token}` } };
                const res = await axios.get(`/api/formateur/users/by-group/${selectedGroup}`, config);
    ...
    if (loading) {
        return (
            <div className={`flex flex-col items-center justify-center h-[60vh] ${isRtl ? 'text-right' : ''}`}>
                ...
                <span className="text-[10px] font-black tracking-[0.4em] uppercase text-slate-400">{t('divisions.loading')}</span>
            </div>
        );
    }
    ...
    return (
        <div className={`space-y-12 fade-up transition-all duration-500 ${isRtl ? 'text-right' : ''}`}>
            <div className={`flex flex-col md:flex-row items-start md:items-end justify-between border-b border-slate-100 pb-12 gap-8 ${isRtl ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className={`flex items-center bg-slate-50 border border-slate-100 rounded-2xl w-full md:w-80 group focus-within:border-[var(--primary)] transition-all px-5 ${isRtl ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`flex gap-6 overflow-x-auto ista-scrollbar pb-6 pt-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
                {availableGroups.map((grp) => (
                    <div
                        key={grp.id}
                        onClick={() => setSelectedGroup(grp.id)}
                        className={`min-w-[280px] flex-shrink-0 p-8 rounded-3xl border transition-all duration-500 group cursor-pointer relative overflow-hidden flex flex-col ${selectedGroup === grp.id
                    ...
                    >
                        <div className={`flex items-center justify-between mb-6 relative z-10 ${isRtl ? 'flex-row-reverse' : ''}`}>
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border transition-all ${selectedGroup === grp.id ? 'bg-[var(--primary)] border-[var(--primary)] text-white shadow-lg shadow-green-500/20' : 'bg-slate-50 border-slate-100 text-slate-400'
                                }`}>
                                <BookOpen className="w-5 h-5" />
                            </div>
                            <span className={`text-[10px] font-black tracking-widest uppercase ${selectedGroup === grp.id ? 'text-[var(--primary)]' : 'text-slate-400'
                                }`}>{grp.id}</span>
                        </div>

                        <h3 className={`text-2xl font-black italic text-[var(--secondary)] tracking-tight mb-2 transition-colors relative z-10 truncate group-hover:text-[var(--primary)] ${isRtl ? 'text-right' : ''}`}>
                            {grp.id}
                        </h3>

                        <p className={`text-[10px] uppercase tracking-widest text-slate-400 font-black mb-8 relative z-10 italic ${isRtl ? 'text-right' : ''}`}>
                            {grp.filiere}
                        </p>

                        <div className={`mt-auto pt-6 border-t border-slate-50 flex items-center justify-between relative z-10 ${isRtl ? 'flex-row-reverse' : ''}`}>
                            <span className="text-[9px] font-black tracking-widest text-slate-300 uppercase">{t('divisions.state')}</span>
                            <div className={`flex items-center gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
                                <div className={`w-1.5 h-1.5 rounded-full ${selectedGroup === grp.id ? 'bg-[var(--primary)] animate-pulse' : 'bg-slate-200'}`}></div>
                                <span className={`text-[9px] font-black tracking-widest uppercase ${selectedGroup === grp.id ? 'text-[var(--primary)]' : 'text-slate-400'
                                    }`}>
            ...
            {/* Student Table */}
            <div className={`pt-8 ${isRtl ? 'direction-rtl' : ''}`}>
                <div className={`flex items-center gap-6 mb-8 ${isRtl ? 'flex-row-reverse text-right' : ''}`}>
                    <h2 className="text-3xl md:text-4xl font-black italic tracking-tighter text-[var(--secondary)] uppercase">{t('nav.members')}</h2>
                ...
                <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden transition-all duration-500">
                    <div className="overflow-x-auto ista-scrollbar">
                        <table className={`w-full text-left border-collapse min-w-[800px] ${isRtl ? 'text-right' : ''}`}>
                            <thead>
                                <tr className="bg-slate-50/50 text-slate-400 text-[9px] font-black uppercase tracking-widest border-b border-slate-100">
                                    <th className="py-6 px-8">{t('accounts.student_name')}</th>
                                    <th className="py-6 px-4">{t('accounts.student_email')}</th>
                                    <th className="py-6 px-4 text-center">{t('divisions.state')}</th>
                                    <th className={`py-6 px-8 ${isRtl ? 'text-left' : 'text-right'}`}>{t('common.last_login')}</th>
                                </tr>
                            </thead>
                                ...
                                {displayedUsers.length > 0 ? displayedUsers.map((student) => (
                                    <tr key={student.id} className="hover:bg-slate-50/30 transition-colors group">
                                        ...
                                        <td className="py-6 px-4">
                                            <div className={`flex items-center gap-2 text-slate-400 group-hover:text-[var(--secondary)] transition-colors ${isRtl ? 'flex-row-reverse' : ''}`}>
                                                <Mail className="w-3.5 h-3.5" />
                                                <span className={`text-[10px] font-bold tracking-widest uppercase ${isRtl ? 'font-mono' : ''}`}>{student.email}</span>
                                            </div>
                                        </td>
                                        <td className="py-6 px-4">
