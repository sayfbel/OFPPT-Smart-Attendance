...
import { SeanceDossierModal } from '../../../components/Modals';
...
const Timelines = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [schedData, formData] = await Promise.all([
                ...
                setFormateurs(formData.formateurs || []);
            } catch (error) {
    ...
    const handleUpdateSeance = async (updatedData) => {
        try {
            await studentService.updateSchedule(updatedData.id, updatedData);
    ...
    const handleCreateSeance = async (newData) => {
        try {
            const resData = await studentService.createSchedule(newData);
