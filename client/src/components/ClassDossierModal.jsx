                            ...
                            <div className={`p-8 bg-slate-50 rounded-[32px] border border-slate-100 grid grid-cols-2 gap-8 ${isRtl ? 'direction-rtl' : ''}`}>
                                <div className="space-y-4 relative">
                                    ...
                                    <div 
                                        ...
                                        className={`w-full bg-white border border-slate-100 rounded-2xl p-5 text-sm font-black text-[var(--primary)] flex justify-between items-center cursor-pointer hover:border-[var(--primary)] transition-all ${isRtl ? 'flex-row-reverse' : ''}`}
                                    >
                                        <span>{startTime || '--:--'}</span>
                                        <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${isStartOpen ? 'rotate-180' : ''}`} />
                                    </div>
                                    {isStartOpen && (
                                        <div className="absolute bottom-full left-0 w-full mb-2 bg-white border border-slate-100 rounded-2xl shadow-2xl z-[60] max-h-48 overflow-y-auto ista-scrollbar animate-in fade-in zoom-in-95 duration-200">
                                            {timeSlots.map(t => (
                                                <div 
                                                    key={t}
                                                    onClick={() => { setStartTime(t); setIsStartOpen(false); }}
                                                    className={`px-6 py-4 text-[10px] font-black tracking-widest uppercase cursor-pointer transition-all hover:bg-green-50 hover:text-[var(--primary)] ${startTime === t ? 'text-[var(--primary)] bg-green-50' : 'text-slate-400'}`}
                                                >
                                                    {t}
                                                </div>
                                            ))}
                                ...
                                <div className="space-y-4 relative">
                                    ...
                                    >
                                        <span>{endTime || '--:--'}</span>
                                        <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${isEndOpen ? 'rotate-180' : ''}`} />
                                    </div>
                                    {isEndOpen && (
                                        <div className="absolute bottom-full left-0 w-full mb-2 bg-white border border-slate-100 rounded-2xl shadow-2xl z-[60] max-h-48 overflow-y-auto ista-scrollbar animate-in fade-in zoom-in-95 duration-200">
                                            {timeSlots.map(t => (
                                                <div 
                                                    key={t}
                                                    onClick={() => { setEndTime(t); setIsEndOpen(false); }}
                                                    className={`px-6 py-4 text-[10px] font-black tracking-widest uppercase cursor-pointer transition-all hover:bg-green-50 hover:text-[var(--primary)] ${endTime === t ? 'text-[var(--primary)] bg-green-50' : 'text-slate-400'}`}
                                                >
                                                    {t}
                                                </div>
                                            ))}
