                ...
                <div className="flex-1 overflow-y-auto ista-scrollbar p-12 pt-8">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                        ...
                        <div className={`space-y-10 ${isRtl ? 'order-2 lg:order-1' : ''}`}>
                            ...
                            <div className={`p-8 bg-slate-50 rounded-[32px] border border-slate-100 flex items-center justify-center ${isRtl ? 'direction-rtl' : ''}`}>
                                <div className="space-y-4 relative w-full">
                                    <label className={`text-[10px] font-black tracking-widest text-slate-400 uppercase flex items-center gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
                                        <Clock className="w-3 h-3" /> {t('modals.dossier.seance_header')}
                                    </label>
                                    <div 
                                    ...
                                    >
                                        ...
                                        <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${isSeanceOpen ? 'rotate-180' : ''}`} />
                                    </div>
                                    {isSeanceOpen && (
                                        <div className="absolute bottom-full left-0 w-full mb-2 bg-white border border-slate-100 rounded-2xl shadow-2xl z-[60] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                            {seanceSlots.map(t => (
                                                <div 
                                                    key={t}
                                                    onClick={() => { setSelectedSeance(t); setIsSeanceOpen(false); }}
                                                    className={`px-6 py-4 text-[10px] font-black tracking-widest uppercase cursor-pointer transition-all hover:bg-green-50 hover:text-[var(--primary)] ${selectedSeance === t ? 'text-[var(--primary)] bg-green-50' : 'text-slate-400'} ${isRtl ? 'text-right' : ''}`}
                                                >
                                                    {t}
                                                </div>
                                            ))}
